defmodule ExOpenAi.ParserTest do
  ...
  test ".parse should decode a response into a struct" do
    response = %{body: "{ \"choices\": [\"Hello, World!\"], \"object\": \"completion\", \"id\": \"cmpl-123\" }", status_code: 200}
    assert {:ok, %Resource{choices: ["Hello, World!"], object: "completion", id: "cmpl-123"}} == parse(response, Resource, nil)
  end

  test ".parse should return an error when response is 401" do
    response = %{body: "{ \"message\": \"Invalid Authentication\" }", status_code: 401}
    assert {:error, %{"message" => "Invalid Authentication"}, 401} == parse(response, Resource, nil)
  end
  ...
  test ".parse should return an error when the server returns 500" do
    ...
    assert {:error, %{"message" => "Internal Server Error"}, 500} == parse(response, Resource, nil)
  end
end
